...
//    connect(_server, SIGNAL(listen()), this, SLOT(readData()));
...
bool QFirmata::openSoc(QString ipAddress, int tcpPort)
{
   // int tcpA = tcpPort.toInt();
//   address.setAddress(ipAddress);

    _socket->connectToHost(ipAddress, tcpPort);
//    _server->listen(address, tcpA);
    if(!_socket->waitForConnected(3000))
    {
        qDebug() << "Connect Error\n";
        return false;
    }
    qDebug()<<"connected to"<<ipAddress<<","<<tcpPort;
    return true;
}
...
/*bool QFirmata::open(QString portName)
{
    ...
    _serial->setBaudRate(115200);//(57600);//
...
{
    if(/*_serial->isOpen() ||*/ _socket->isValid())
    {
        _socket->write(bytes);
        //_serial->write(bytes);
